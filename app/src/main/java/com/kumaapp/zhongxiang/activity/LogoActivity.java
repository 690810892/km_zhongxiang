...
import com.kumaapp.zhongxiang.util.LocationUtils;
...
public class LogoActivity extends BaseActivity implements AMapLocationListener {
    ...
    protected void callBackForServerSuccess(HemaNetTask netTask,
        ...
        switch (information) {
            case INIT:
                ...
                BaseApplication.getInstance().setSysInitInfo(infor);
                toAdvertisement();
                break;
            case CLIENT_LOGIN:
    ...
    private void toAdvertisement() {
        String last_version = XtomSharedPreferencesUtil.get(mContext, "version");
        if (isNull(last_version) || !last_version.equals(HemaUtil.getAppVersionForSever(mContext))) {//第一次或者升级版本
            XtomSharedPreferencesUtil.save(mContext, "isShowed", "false");
        }
        XtomSharedPreferencesUtil.save(mContext, "version", HemaUtil.getAppVersionForSever(mContext));
        checkLogin();
    }
    ...
    private void toMain() {
//        Intent it = new Intent(mContext, Maina.class);
//        startActivity(it);
//        finish();
//        stopLocation();
    }
